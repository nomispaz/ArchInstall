...
#+END_SRC
** window manager sway
#+begin_src bash :tangle 5_install_windowmanager_sway.sh
...
Directly change desktop file so that custom start skript is run
#+begin_src bash :tangle 5_install_windowmanager_sway.sh
...
#+begin_src bash :tangle 00_wrapper_kde_qtile_sway_nvidia.sh

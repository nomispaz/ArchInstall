...
if [ -n "$fish_version" ]; then
  ...
  set UUID (grep -E '^[^#].*\s/\s.*btrfs.*subvol=(/)?root' $FSTAB_FILE | awk '{print $1}' | sed 's/^UUID=//')
else
  echo "Nont running in Fish shell"
  UUID=$(grep -E '^[^#].*\s/\s.*btrfs.*subvol=(/)?root' /etc/fstab | awk '{print $1}' | sed 's/^UUID=//')
fi
